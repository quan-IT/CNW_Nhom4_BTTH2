...
<head>
    ...
    <style>
        /* Nếu bạn chưa có CSS, thêm vài style tối giản để tránh vỡ layout (tùy chỉnh tiếp trong file CSS của bạn) */
        .container{max-width:1100px;margin:30px auto;padding:0 16px}
        .courses-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:18px}
        .course-card{display:block;text-decoration:none;color:inherit;border:1px solid #eee;border-radius:8px;overflow:hidden;background:#fff}
        .course-card .thumb img{width:100%;height:160px;object-fit:cover;display:block}
        .course-card .body{padding:12px}
        .course-card .title{font-size:16px;margin:0 0 8px}
        .course-card .instructor{margin:0 0 8px;color:#666;font-size:13px}
        .course-card .info{display:flex;gap:8px;align-items:center;font-size:13px;color:#444;margin-bottom:8px}
        .course-card .price{font-weight:700}
    </style>
</head>
        ...
        <div class="courses-grid">
            <?php foreach ($courses as $c): ?>
                <?php
                    ...
                    $image = $c['image'] ?? 'assets/default.png';
                    $instructorName = $instructors[$c['instructor_id']] ?? 'Ẩn danh';
